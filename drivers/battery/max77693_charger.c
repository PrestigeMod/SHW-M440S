...
static void max77693_charger_reg_init(struct max77693_charger_data *chg_data)
{
	 ...
	 *  For GC1 Model  PRMV( Primary Charge Regn. Voltage) = 4.225V.
	 * Actual expected regulated voltage needs to be 4.2V but due to
	 * internal resistance and cicuit deviation we need to set the
	 * benchmark a bit higher(4.22V).
	 */
#if defined(CONFIG_MACH_GC1)
	reg_data &= (~MAX77693_CHG_PRM_MASK);
	reg_data |= (0x17 << MAX77693_CHG_PRM_SHIFT);
