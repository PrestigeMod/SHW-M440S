		...
		struct modem_shared *msd)
{
	...
	modemctl->storage.cnt = 0;
	modemctl->storage.addr = kzalloc(MAX_MIF_BUFF_SIZE, GFP_KERNEL);
	if (!modemctl->storage.addr) {
		mif_err("IPC logger buff alloc failed!!\n");
		return NULL;
	} else {
		modemctl->storage.utc_zone = modemctl->storage.addr;
		modemctl->storage.addr += MAX_MIF_UTC_SIZE;
	}
